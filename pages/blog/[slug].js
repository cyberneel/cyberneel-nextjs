...
import { getArticleSlug, getArticleFromSlug, getAllArticles, extractToc, pickRelated } from '../../src/utils/mdx';
import { tagSlug } from '../../src/utils/tags';

const mdxComponents = { Image, SectionTitle, Text, TurntableViewer, ImageMDX, Font: FontWrapper, pre: Pre };

export default function Article({ post: { source, frontmatter }, toc, related }) {
  return (
    <>
      <ReadingProgress />
      <Seo
            ...
            {frontmatter.tags?.length > 0 && (
              <div className="mb-5 flex flex-wrap gap-2">
                {frontmatter.tags.map((t) => (
                  <Link key={t} href={`/blog/tag/${tagSlug(t)}`} className="chip transition-colors hover:bg-[color-mix(in_srgb,var(--accent)_18%,transparent)]">
                    {t}
                  </Link>
                ))}
              </div>
